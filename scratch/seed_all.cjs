...
async function run() {
  ...
  console.log('🚀 Seeding demo merchants for all categories & subcategories into AWS Aurora PostgreSQL...');
  ...
  for (let i = 0; i < CATEGORIES_HIERARCHY.length; i++) {
    ...
    const existingCat = await client.query('SELECT id FROM categories WHERE slug = $1 LIMIT 1', [cat.slug]);
    let effectiveCatId = cat.id;
    if (existingCat.rows && existingCat.rows.length > 0) {
      effectiveCatId = existingCat.rows[0].id;
      await client.query(
        `UPDATE categories SET name = $1, icon = $2, sort_order = $3 WHERE id = $4`,
        [cat.name, cat.icon, i + 1, effectiveCatId]
      );
    } else {
      await client.query(
  ...
  console.log('✅ Successfully seeded ' + totalCategories + ' categories with ' + totalMerchants + ' demo merchants into AWS Aurora PostgreSQL!');
  client.release();
  await pool.end();
}
